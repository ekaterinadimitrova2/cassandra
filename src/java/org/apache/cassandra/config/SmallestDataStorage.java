...
public abstract class SmallestDataStorage extends DataStorageSpec
{
    protected SmallestDataStorage(String value, DataStorageUnit smallestUnit, boolean isInt)
    {
        ...
        validateValue(value, this::quantity, this.unit(), smallestUnit, isInt);
    }
    ...
    protected SmallestDataStorage(long quantity, DataStorageUnit unit, DataStorageUnit smallestUnit, boolean isInt)
    {
        ...
        validateValue(quantity, unit, smallestUnit, isInt);
    }

    private static void validateValue(String value, LongSupplier valueSupplier, DataStorageUnit unit, DataStorageUnit smallestUnit, boolean isInt)
    {
        if (!isInt)
            validateLong(value, valueSupplier, unit, smallestUnit);
        else
            validateInt(value, valueSupplier, unit, smallestUnit);
    }

    private static void validateValue(long quantity, DataStorageUnit unit, DataStorageUnit smallestUnit, boolean isInt)
    {
        if (!isInt)
    ...
    private static void validateLong(String value, LongSupplier valueSupplier, DataStorageUnit unit, DataStorageUnit smallestUnit)
    {
        if (smallestUnit.convert(valueSupplier.getAsLong(), unit) == Long.MAX_VALUE)
    ...
    private static void validateInt(String value, LongSupplier valueSupplier, DataStorageUnit unit, DataStorageUnit smallestUnit)
    {
        if (smallestUnit.convert(valueSupplier.getAsLong(), unit) > Integer.MAX_VALUE)
            throw new ConfigurationException("Invalid data storage: " + value + ". It shouldn't be more than " +
