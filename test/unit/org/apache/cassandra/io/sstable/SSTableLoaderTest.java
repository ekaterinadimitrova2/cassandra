...
public class SSTableLoaderTest
{
    ...
    {
        File dataDir = dataDir(KEYSPACE1, CF_STANDARD2);
    ...
    {
        File dataDir = dataDir(KEYSPACE1, CF_BACKUPS);
    ...
    private File dataDir(String ks, String cf)
    {
        File dataDir = new File(tmpdir.getAbsolutePath() + File.separator + ks + File.separator + cf);
        assert dataDir.mkdirs();
        //make sure we have no tables...
        assertEquals(dataDir.listFiles().length, 0);
        return dataDir;
    }
