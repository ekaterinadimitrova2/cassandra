...
{
    static Path configTemplate;
    @BeforeClass
    public static void beforeClass() throws URISyntaxException
    {
        Path p = Paths.get(YamlConfigurationLoader.getStorageConfigURL().toURI());
        configTemplate = Paths.get(p.getParent().toString(), "cassandra-dupes-template.yaml");
    }
    ...
    public void before()
    {
        ...
        ALLOW_CONFLICTING_CONFIG_VALUES.setBoolean(false);
    }
    ...
    {
        testYaml("old: [enable_user_defined_functions: true], new: [user_defined_functions_enabled: false]", true,
                 "enable_user_defined_functions: true",
                 "user_defined_functions_enabled: false");

        testYaml("nothing", false,
                 "enable_user_defined_functions: true",
                 "user_defined_functions_enabled: true");

        ALLOW_DUPLICATE_CONFIG_KEYS.setBoolean(false);

        testYaml("Config contains duplicate keys", true,
                 "enable_user_defined_functions: true",
                 "user_defined_functions_enabled: true");
    ...
    public DataStorageSpec internode_socket_send_buffer_size = new DataStorageSpec("0B");
         */
        Predicate<String> predicate = (s) -> s.contains("[internode_send_buff_size_in_bytes: 3], new: [internode_socket_send_buffer_size: 5B]") &&
                                             s.contains("[internode_socket_send_buffer_size_in_bytes: 2], new: [internode_socket_send_buffer_size: 5B]");
    ...
    private static void testYaml(Predicate<String> exceptionMsgPredicate, boolean expectFailure, String message, String ... toAdd) throws IOException
    {
        ...
        try
        {
            List<String> lines = Lists.newArrayList(Files.readAllLines(configTemplate));
