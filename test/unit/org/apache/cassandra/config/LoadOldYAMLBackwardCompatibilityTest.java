...
public class LoadOldYAMLBackwardCompatibilityTest
{
    ...
    public void testConfigurationLoaderBackwardCompatibility()
    {
        ...
        assertEquals(DataStorageSpec.inMegabytes(16), config.native_transport_max_frame_size);
        assertEquals(DataStorageSpec.inMegabytes(256), config.max_value_size);
        assertEquals(DataStorageSpec.inKilobytes(64), config.column_index_size);
        assertEquals(DataStorageSpec.inKilobytes(2), config.column_index_cache_size);
        assertEquals(DataStorageSpec.inKilobytes(5), config.batch_size_warn_threshold);
        assertEquals(DataRateSpec.inMebibytesPerSecond(64), config.compaction_throughput);
        assertEquals(DataStorageSpec.inMegabytes(50), config.min_free_space_per_drive);
        assertEquals(DataRateSpec.inMebibytesPerSecond(200), config.stream_throughput_outbound);
        ...
        assertEquals(DataStorageSpec.inMegabytes(32), config.commitlog_segment_size);
        assertNull(config.periodic_commitlog_sync_lag_block);  //Integer
        assertNull(config.max_mutation_size);
        assertEquals(DataStorageSpec.inMegabytes(0), config.cdc_total_space);
        ...
        assertEquals(DataStorageSpec.inKilobytes(1024), config.hinted_handoff_throttle);
        assertEquals(DataStorageSpec.inKilobytes(1024), config.batchlog_replay_throttle);
        assertEquals(DurationSpec.inMilliseconds(10000), config.hints_flush_period);
        assertEquals(DataStorageSpec.inMegabytes(128), config.max_hints_file_size);
        assertEquals(DataStorageSpec.inKilobytes(10240), config.trickle_fsync_interval);
        assertEquals(DataStorageSpec.inMegabytes(50), config.sstable_preemptive_open_interval);
        assertNull( config.key_cache_size);
        assertEquals(DataStorageSpec.inMegabytes(0), config.row_cache_size);
