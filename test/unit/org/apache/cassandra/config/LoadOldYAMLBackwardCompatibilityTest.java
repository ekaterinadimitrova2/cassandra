...
public class LoadOldYAMLBackwardCompatibilityTest
{
    ...
    public void testConfigurationLoaderBackwardCompatibility()
    {
        ...
        assertEquals(new DataRateSpec.IntMebibytesPerSecond(64), config.compaction_throughput);
        assertEquals(new SmallestDataStorage.IntMebibytes(50), config.min_free_space_per_drive);
        assertEquals(new DataRateSpec.IntMebibytesPerSecond(23841858).toString(), config.stream_throughput_outbound.toString());
        assertEquals(DataRateSpec.IntMebibytesPerSecond.megabitsPerSecondInMebibytesPerSecond(200000000).toString(), config.stream_throughput_outbound.toString());
