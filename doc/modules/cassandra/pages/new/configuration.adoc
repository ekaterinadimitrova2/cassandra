...
- Parameters of type data storage, duration and data rate cannot be set to Long.MAX_VALUE (former parameters of int type).
The only reason to see this number assigned to a parameter will be a bug. That number is used during conversion between units to
prevent an overflow from happening.
