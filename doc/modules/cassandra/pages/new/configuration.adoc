= Liberate cassandra.yaml Parameters' Names from Their Units
...
3) Backward compatibility framework to support the old names and lack of units’ support at least until the next major release.


== Renamed Parameters

The community took the decision to give the chance to people to decide what units to use for their Cassandra parameters of type duration, data storage, and data rate.
...
The work in https://issues.apache.org/jira/browse/CASSANDRA-15234[CASSANDRA-15234] was already quite big, so we decided to introduce the notion of the smallest allowed unit per parameter for duration and data storage parameters. What does this mean?
Currently, Cassandra internals still use the old unit for parameters and in case, for example, you have seconds used internally, but you want to add a value in nanoseconds in cassandra.yaml,
...
to be manipulated with the smallest supported by Cassandra unit. Series of tickets to assess and migrate to the smallest unit
...
Another TO DO is to add new JMX methods supporting the new format. This might never happen though if virtual tables provide
support to change our configuration sooner than later. Discussions and preliminary work are in the making.
