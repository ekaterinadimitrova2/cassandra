...
granularity of archiving; 8 or 16 MiB is reasonable. Max mutation size is
also configurable via `max_mutation_size` setting in `cassandra.yaml`.
The default is half the size `commitlog_segment_size` in bytes.
